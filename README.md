# Independent replication of the performance experiments on Matjuice from Vincent Foley-Bourgon's Master Thesis
...
## Comments regarding specific benchmark implementations used:
...
* I significantly changed the input of the fft benchmark. It now computes the signals in the frequency-domain of an addition of two sinusoidal functions in the time-domain. The verification of the output checks that we indeed find two signals in the frequency domain. It was much simpler than trying to compare the output of fft on random noise which was basically impossible. The performance results obtained are still congruent with the results in the thesis. Note also the fft benchmark used in the original performance evaluation corresponds to the fft_simple benchmark of Ostrich2 and not the fft benchmark, which originally comes from the Ostrich suite. The experiment in this respository renames the 'fft-simple' benchmark to 'fft' to follow the conventions of the original study;
* The dich benchmark has also been renamed 'dirich', because the former may sound politically incorrect depending on the pronunciation used... Again the experiment file, renames it to 'dich' when downloading the benchmark to follow the conventions of the original study.
* The makechange() benchmark increases the number of times the benchmark is executed rather than increasing the size of the amount computed, which would seem to be the obvious way to scale the input size because the memory size used would increase. At the moment, the benchmark simply runs for longer by computing the same result multiple times. I understand that may be inevitable for some benchmarks for which different inputs do not modify the amount of computation or memory used, but I don't think it is was would be expected for makechange().
* The crni() benchmark is the only one for which Matjuice produces a result that is different from Mathworks' MATLAB. Somehow a bug in the implementation produces NaN results in the last elements of the output. This should be investigated and fixed.
